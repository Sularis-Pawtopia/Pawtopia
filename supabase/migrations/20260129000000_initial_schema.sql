-- =============================================
-- PAWTOPIA DATABASE SCHEMA - COMPLETE
-- PostgreSQL + Supabase with Storage Buckets
-- =============================================
...
CREATE TYPE user_role AS ENUM ('admin', 'regular_user', 'adopter', 'volunteer', 'shelter', 'ngo', 'city_pound');
...
CREATE TYPE post_type AS ENUM ('adoptable', 'lost_pet', 'event', 'story', 'feed', 'general', 'success_story');
...
-- =============================================
-- USER ROLES (Simple junction table for multiple roles)
-- =============================================
...
CREATE POLICY "Allow public to view pet images"
ON storage.objects FOR SELECT
TO public
USING (bucket_id = 'pet-images');

CREATE POLICY "Allow users to update own pet images"
ON storage.objects FOR UPDATE
TO authenticated
USING (bucket_id = 'pet-images' AND auth.uid()::text = (storage.foldername(name))[1]);

CREATE POLICY "Allow users to delete own pet images"
...
CREATE POLICY "Allow shelters to update own event images"
ON storage.objects FOR UPDATE
TO authenticated
USING (
  bucket_id = 'event-images' AND 
...
CREATE POLICY "Allow shelters to delete own event images"
ON storage.objects FOR DELETE
TO authenticated
USING (
  bucket_id = 'event-images' AND 
...
CREATE POLICY "Allow shelters to view adopter documents"
ON storage.objects FOR SELECT
TO authenticated
USING (
...
CREATE POLICY "Allow public to view stories"
ON storage.objects FOR SELECT
TO public
USING (bucket_id = 'stories');

CREATE POLICY "Allow users to update own stories"
ON storage.objects FOR UPDATE
TO authenticated
...
-- Example:
-- INSERT INTO users (id, email, username, role, is_verified) VALUES (...);
